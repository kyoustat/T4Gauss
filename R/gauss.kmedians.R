...
gauss.kmedians <- function(glist, k=2, type=c("wass2"), maxiter=100, nthreads=1,
  ...
  center.old = gauss.kmedians.center(glist, label.old, myk, mytype, nCores)
...
#' @keywords internal
#' @noRd
gauss.kmedians.center <- function(glist, label, k, type, nthreads){
  label  = round(label)
  n      = length(glist)
  k      = round(k)
  ...
  for (i in 1:k){
    ...
    if (length(idnow)==1){
      ...
      gparts = glist[idnow]
      if (all(type=="wass2")){
        centers[[i]] = median_selection(gparts, method="wass2", par.iter=100, par.eps=1e-6, nCores=nthreads)
      }
    }
  }
  return(centers)
